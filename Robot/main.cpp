...
void Jump_Timer(int val) {
	...
	if (mariocontroller == 3 || mariocontroller == 7) { // 設定馬力歐跳起到著地的時間
		...
		if ((currentTime - deltatime) >= time_for_a_jump) { // 要著地了，看向哪一方
			...
			if (mariocontroller == 3) {
				mariocontroller = 0; // 著地後，看向右方
			}
			else if (mariocontroller == 7) {
				mariocontroller = 4; // 著地後，看向左方
			}
		}
...
//         跳7,走65,站4:左   右:0站,12走,3跳
void Keyboard(unsigned char key, int x, int y) {
	switch (key) {
	case 'w':
	case 'W':
		if (mariocontroller == 1 || mariocontroller == 0) { // 上一個時刻面向右邊
			
			if (mariocontroller == 1) {
				is_move_when_jump = 1;
			}
			deltatime = currentTime;
			mariocontroller = 3; // 換成向右跳躍的圖
		}
		else if(mariocontroller == 5 || mariocontroller == 4){ // 上一個時刻面向左邊
			
			if (mariocontroller == 5) {
				is_move_when_jump = 2;
			}
			deltatime = currentTime;
			mariocontroller = 7; // 換成向左跳躍的圖
		}
		 // 跳躍
		break;
	case 'a':
	case 'A':
		if (mariocontroller == 7) { // 上一個時刻向左跳
			mariocontroller = 7; // 仍然向左
			is_move_when_jump = 2; // 邊跳邊向左移動
		}
		else if (mariocontroller == 3) { // 上一個時刻向右跳
			mariocontroller = 7; // 改成向左
			is_move_when_jump = 2; // 邊跳邊向左移動
		}
		else {
			mariocontroller = 5; // 向左走
	...
	case 'D':
		if (mariocontroller == 7) { // 上一個時刻向左跳
			mariocontroller = 3; // 改成向右
			is_move_when_jump = 1; // 邊跳邊向右移動
		}
		else if (mariocontroller == 3) { // 上一個時刻向右跳
			mariocontroller = 3; // 仍然向右
			is_move_when_jump = 1; // 邊跳邊向右移動
		}
		else {
			mariocontroller = 1; // 向右走
